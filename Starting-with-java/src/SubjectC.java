 ...
 *
 */
public class SubjectC implements Subjects//extends Subjects 
{
		...
		public ArrayList<String> allocateCourse(int priority,ArrayList<Student> students,ArrayList<String> courseParticipants)
		{
			try
			{
				for(int counter=0;counter<students.size();counter++)
				{
					   ...
					   if(student.getPreference_C()==priority && AllocatedSeats<10 && student.getNoOfAllocatedCourses()<3)
					   {
						   courseParticipants.add(student.getName());
						   AllocatedSeats++;
						   student.setPreferenceScore(student.getPreferenceScore()+student.getPreference_C());
			...
			{
				System.out.println("Problem while Allocating Subject C! Program will terminate");
